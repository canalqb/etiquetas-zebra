...
Para unificar o layout e garantir a qualidade visual:
- **CSS Centralizado**: É **terminantemente proibido** o uso de CSS inline ou blocos `<style>` dentro dos arquivos PHP de páginas, a menos que solicitado explicitamente pelo usuário. Todo o CSS novo deve ser alimentado no arquivo `css/styles.css` do projeto.
- **JS Centralizado**: Da mesma forma, utilize preferencialmente o arquivo `js/styles.js` para scripts globais e interações.
