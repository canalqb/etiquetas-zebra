...
## 🎨 **CSS Centralizado e Estilização**

### 📋 **Mandatos de Estilo:**
- **NUNCA** usar CSS inline (`style="..."`).
- **NUNCA** incluir blocos `<style>` dentro de arquivos de páginas injetadas (a menos que o usuário peça).
- **SEMPRE** alimentar o arquivo `css/styles.css` do site com os novos estilos.
- **UTILIZAR** as variáveis CSS definidas em `regra_php_ui_template_design_system.md`.

### 📋 **Checklist de Validação CSS:**
- [ ] O arquivo PHP da página contém algum CSS inline? (Deve ser removido)
- [ ] O arquivo PHP da página contém algum bloco `<style>`? (Deve ser migrado para `css/styles.css`)
- [ ] Os estilos utilizam variáveis CSS do Design System?
```php
<style>
/* Animação padrão */
.animate-fade {
...
- [ ] Garantir que **NENHUM** estilo está inline ou interno (tudo no `css/styles.css`)
