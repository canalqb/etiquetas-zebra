...
|------|-----|----------|
| 2026-08-20 | **Validação física na Zebra** do fluxo de impressão vertical/horizontal. O código já está corrigido e validado em render: saída ZPL sempre horizontal (`^PW800 ^LL400`), rotações 0/90/180/270 conferidas por pixels na Virtual ZPL Printer (bbox conteúdo 0..798 × 0..398, todos os elementos dentro dos limites, margem mínima 24 dots no teste 100x50 vertical). | Falta apenas conferir o resultado na impressora física ZT411 (hardware indisponível no momento). Durante esse teste, revisar `^FO` vs `^FT` (canto vs linha de base). |

> Resolvidos e removidos deste registro (2026-08-20): perda de foco + `NotFoundError` na árvore do Lote Especial (guarda anti-reentrância `_renderingTreeUI` + restauração de foco/caret em `renderBatchSpecialTreeUI()`, validado via CDP); correção das fórmulas de origem para rotação em `gerarZPLBloco()` (validado por pixels); acessibilidade master_rules (ARIA nos 9 modais, `btn-close` com `aria-label`, semântica `<main>/<header>/<aside>/<section>/<nav>`, sem `console.log`) — validada em Chrome headless com 0 erros de página do app.
...
4. ~~**Múltiplos modelos em abas**~~ ✅ **Resolvido (2026-08-20)**: barra de abas acima do canvas (`#tabs-modelos`) — uma aba por modelo salvo no cache, aba ativa destacada, clique carrega o modelo (`carregarModeloPorNome()`), botão **+** cria etiqueta em branco (com confirmação). Sincronizada ao salvar/excluir/carregar/renomear.
5. ~~**Galeria de templates prontos**~~ ✅ **Resolvido (2026-08-20)**: 3 templates na Seção "Salvar / Exportar" — **Envio/Caixa**, **Estoque** e **Série/QR** (100×50 mm, textos + linha + QR + barcode). Um clique aplica o template no editor (`aplicarModelo()`), nomeia o modelo e mantém histórico (Ctrl+Z funcional).
6. **Backup/compartilhamento em nuvem** — ⚠️ parcial (2026-08-20): ✅ **Link compartilhável** — botão de corrente na Seção 1 serializa o modelo (nome+tamanho+rotação+elementos) em Base64URL na hash `#modelo=...`; ao abrir o link o modelo é aplicado automaticamente e o hash é limpo (em http/https; em `file://` o Chrome pode manter o fragmento, sem efeito prático). Limite de ~8 KB por link (modelos maiores devem usar .BIN). Restante: exportação JSON de todos os modelos / IndexedDB.
7. ~~**Mobile** — habilitar salvar/exportar também no offcanvas~~ ✅ **Resolvido (2026-08-20)**: o offcanvas mobile agora é **espelho real do painel desktop** — as seções "Salvar / Exportar (.BIN)" e "Geração em Lote" são movidas para o offcanvas em telas < 992px e devolvidas ao desktop acima disso (`Mobile Panel Mirror`, mesmo DOM/IDs/listeners, zero duplicação). A antiga Seção "Configuração da Etiqueta" com inputs duplicados (`m-*`) e o script de sync legado foram removidos. Validado via CDP com viewports 390px e 1280px: salvar modelo, import/export, árvore de lote (+ subnível) e gerar lote (`^SN`+`^PQ`) funcionam dentro do offcanvas; retorno ao desktop restaura a ordem original das seções.
8. **Contador de etiquetas por rolo + estimativa** — ⚠️ quase total (2026-08-20): ✅ contador implementado no modal de Tamanho — comprimento do rolo (m) + espaço entre etiquetas (mm, default 3) → `≈ N etiquetas` na orientação de saída (considera rotação 90/270). Restante: campo opcional de preço/rolo para custo por etiqueta.
9. **ZPL avançado a partir dos PDFs** (ver seção abaixo): já aplicados `^SN`, `^FB`, `^PQ` e agora `^PM` (espelho) + `^LR` (reverso branco-sobre-preto) como toggles no header, persistidos e incluídos no prologue dos geradores vetorial e imagem; `^MD` em uso. Restam: `^FH`/`^FE`, fontes TrueType `^A@`.
10. **Acessibilidade/qualidade** — ✅ concluído (2026-08-20): todos os 9 modais com `role="dialog"` + `aria-modal="true"` + `aria-labelledby`; todos os `btn-close` com `aria-label="Fechar"` (incl. `#modal-limpar`, que não tinha botão de fechar, e `#insp-btn-deselect`); semântica aplicada (`<main>`, `<header>`, `<aside>` no offcanvas mobile, 5× `<section class="accordion-item">`, `<nav aria-label="Abas do Studio">` nos tabs); sem `console.log/warn/error` no app. Validado em Chrome headless: 0 erros de página do app, tags balanceadas, ZPL gerado corretamente. Ruído externo conhecido: `adsbygoogle.push() error` (script de anúncios do Google, fora do controle do app). ✅ **Tema claro/escuro** adicionado (botão lua/sol no header, `data-bs-theme` Bootstrap 5.3 + variáveis CSS `--zebra-*`; papel da etiqueta e réguas permanecem brancos de propósito). Restam apenas: lazy-load de fontes/ícones.
